  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/compose', {
        ...
        body: JSON.stringify({ notes, style, instruments }),
      });
          ...
          <div className="space-y-4">
            {notes.length === 0 && (
              <div className="rounded-lg border border-amber-500/20 bg-amber-500/8 px-4 py-3 text-sm text-amber-400/80">
                ⬆ まずステップ1でメロディを録音してください
              </div>
            )}

            {notes.length > 0 && appState !== 'generating' && (
            ...
            <button
              ...
              disabled={notes.length === 0 || appState === 'generating'}
    ...
    <div
      className={`mb-6 rounded-2xl border p-6 transition-all duration-300
        ${highlight
          ...
          : active
            ? 'border-[#1a1a3a] bg-[#0d0d1f]'
            : 'border-[#111128] bg-[#090915] opacity-60'
        }`}
    >
      <div className="mb-5 flex items-center gap-3">
        <StepBadge n={step} active={active} done={done} />
        <h2 className={`font-semibold ${active ? 'text-slate-200' : 'text-slate-600'}`}>
          {title}
        </h2>
      </div>
      {children}
